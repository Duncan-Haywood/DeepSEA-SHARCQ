...
class API: 
    ...
    def create_api(self):
        ...
        async def get_upload_signed_url(Body):
            method_parameters=dict(Bucket=os.get_environ("ZIPPED_BUCKET"), Key=None) #TODO decide on key for files
            url = self.aws_util.generate_presigned_url("put", method_parameters, self.expires_in)
            return dict(upload_url=url)

        @self.app.post('/download_url/')
        async def get_download_signed_url(Body):
            method_parameters=dict(Bucket=os.get_environ("ZIPPED_RESULTS_BUCKET"), Key=None) #TODO decide on key for files
            url = self.aws_util.generate_presigned_url("get", method_parameters, self.expires_in)
