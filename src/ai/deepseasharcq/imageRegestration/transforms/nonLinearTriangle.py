...
class Triangle_transform(Transform):
    ...
    def delaunay_transform(self, imgb, imga, subdiv, mapping):
        ...
        for t in triangleList:
            ...
            tri_1 = np.array([pt11, pt21, pt31], dtype=np.float32)

            triangle_contours += [tri_1]

            tri_2 = np.array([pt12, pt22, pt32], dtype=np.float32)
            ...
            cropped_triangle = cv2.bitwise_and(
                cropped_triangle, cropped_triangle, mask=cropped_tr1_mask
            )
            ...
            final_ouput[y : y + h, x : x + w] = cv2.add(
                final_ouput[y : y + h, x : x + w], warped_triangle
            )
        ...
        for p in point:
            for i, c in enumerate(triangle_contour_set):
                ...
                if cv2.pointPolygonTest(c, tuple(p), False) == 1:
                    ...
                    transformed_points += [
                        (triangle_transform[i][0]).dot(points_ones.T).T.reshape(2)
                        + np.array(triangle_transform[i][2])
                    ]
                    break
    ...
    # Abstract methods
    def buildTransform(
        self, telemetry: List["Points"], imga: "np.array", imgb: "np.array"
    ):
        ...
        mapping = {
            tuple(p1[::-1]): telemetry[0][i, ::-1] for i, p1 in enumerate(telemetry[1])
        }
        self.contours, self.transform, self.final_ouput = self.delaunay_transform(
            imga.copy(), imgb.copy(), self.subdiva.subdivb, mapping=mapping
        )
    ...
    def preformTransform(self, dataPoints: "Points", return_final_image=False):
        ...
        return self.preform_triangle_affine(
            dataPoints.to_numpy(), self.contours, self.transform
        )
